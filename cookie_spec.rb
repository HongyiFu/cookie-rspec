...
describe Cookie do
  ...
  describe "#status" do
    ...
    context "when unbaked" do
      it "is `:doughy`" do
        expect(Cookie.new("some_type").status).to eq :doughy
      end
    end

    context "when baked for less than 7 minutes" do
      it "is `:doughy`" do
        expect(Cookie.new("some_type").bake!(6).status).to eq :doughy
      end
    end

    context "when baked for at least 7 but less than 10 minutes" do
      it "is `:almost_ready`" do
        expect(Cookie.new("some_type").bake!(9).status).to eq :almost_ready
      end
    end

    context "when baked for at least 10 but less than 12 minutes" do
      it "is `:ready`" do 
        expect(Cookie.new("some_type").bake!(11).status).to eq :ready
      end
    end

    context "when baked for at least 12 minutes" do
      it "is `:burned`" do
        expect(Cookie.new("some_type").bake!(12).status).to eq :burned
      end
    end
  end
end